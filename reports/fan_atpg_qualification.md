...
Full stdout/stderr, exit files, copied patterns, and checksum manifest are under `artifacts/raw/tool_qualification/fan_atpg/`. The `.pat` writer at the recorded commit (`pkg/core/src/pattern_rw.cpp`, `PatternWriter::writePat`) emits three ordered header lines (PI, PPI, PO), then `BASIC_SCAN`, count, and seven fields per pattern: `PI1 | PI2 | PPI | SI | PO1 | PO2 | PPO`. The PPI field supplies logical target scan-FF states. The qualification itself did not yet validate a PPI-to-physical-FF map; subsequent s5378 integration did so as described below.

The supplied `mod_netlist` circuits contain pre-stitched `SDFF_X1` cells with names matching the PPI header, and ORFS Nangate45 Liberty contains `SDFF_X1`. Subsequent Phase-0 integration established 179-way s5378 and 211-way s9234 PPI-to-placed-FF identity maps and verified scan-only rewires for both; see their `artifacts/derived/<design>/identity_check.json` and `artifacts/derived/phase0/smoke_<design>/nearest_neighbor.rewire_verification.json`. s27 and s15850 remain ATPG-qualified only.
